...
class DataConfig:
    ...
    tooltips: TooltipConfig | None = None
...
def load_config(path: str | Path | None = None) -> DataConfig:
    ...
    tt = raw.get("tooltips") or {}
    tooltips = TooltipConfig(
        graph_node = list(tt.get("graph_node", ["location_name","location_type","id","degree"])),
        map_node   = list(tt.get("map_node",   ["location_name","location_type","id","degree","lat","lon"])),
    )
